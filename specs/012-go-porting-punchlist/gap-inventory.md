...
| GAP-005 | Functional | Medium | Conversation pagination defaults alignment | Rust: page_size=0 => unbounded; honors pageSize query param | Go now honors pageSize and returns page_size=0 for unbounded | Verified via tests: meta.page_size == 10 when pageSize=10; meta.page_size == 0 when pageSize=0 | Closed |
