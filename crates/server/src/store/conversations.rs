...
fn convo_id(channel: &Channel, from: &str, to: &str) -> (String, String) {
	...
	let key = format!("{}:{}<->{}", match channel { Channel::Sms => "sms", Channel::Mms => "mms", Channel::Email => "email" }, a, b);
	let id = key.clone();
	(id, key)
}
...
pub fn list(page: u32, page_size: u32) -> (Vec<ConversationDto>, u64) {
	...
	let start = if page_size == 0 { 0 } else { ((page.max(1) - 1) * page_size) as usize };
	let end = if page_size == 0 { items.len() } else { (start + page_size as usize).min(items.len()) };
...
pub fn list_messages(
	...
	let start = if page_size == 0 { 0 } else { ((page.max(1) - 1) * page_size) as usize };
	let end = if page_size == 0 { msgs.len() } else { (start + page_size as usize).min(msgs.len()) };
